...
public class Starve implements CommandExecutor {
	...
	public Starve(Main plugin) {
		this.plugin = plugin;
		plugin.getCommand("starve").setExecutor(this);
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
	    ...
	    if (this.config.getConfigurationSection("commands").getConfigurationSection("starve") != null && this.config.getConfigurationSection("commands").getConfigurationSection("starve").getBoolean("enabled") == true) {
			if (!(sender instanceof Player)) {
				if (args.length > 0) {
					if (sender.getServer().getPlayer(args[0]) != null) {
						Player other = (Player) sender.getServer().getPlayer(args[0]);
						other.sendMessage(handler.getCaption("WasStarved"));
			...
			if (p.hasPermission("catx.starve")) {
				if (args.length > 0 && p.hasPermission("catx.starve.others")) {
					if (sender.getServer().getPlayer(args[0]) != null) {
						Player other = (Player) sender.getServer().getPlayer(args[0]);
						if (other.getName() == p.getName()) {
							p.sendMessage(handler.getCaption("WasStarved"));
							p.setFoodLevel(0);
							return true;
						}
						other.sendMessage(handler.getCaption("WasStarved"));
