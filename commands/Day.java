...
public class Day implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
	    ...
	    if (this.config.getConfigurationSection("commands").getConfigurationSection("day") != null && this.config.getConfigurationSection("commands").getConfigurationSection("day").getBoolean("enabled") == true) {
			ConsoleCommandSender console = sender.getServer().getConsoleSender();
			if (console != null) {
				sender.sendMessage(handler.getCaption("TimeSet").replace("{0}", "day").replace("{1}",
						sender.getServer().getWorlds().get(0).getName()));
				sender.getServer().getWorlds().get(0).setTime(500);
				return true;
			}
			Player p = (Player) sender;
			if (p.hasPermission("catx.day")) {
				p.sendMessage(handler.getCaption("TimeSet").replace("{0}", "day").replace("{1}", p.getWorld().getName()));
