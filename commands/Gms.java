...
public class Gms implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
	    ...
	    if (this.config.getConfigurationSection("commands").getConfigurationSection("gms") != null && this.config.getConfigurationSection("commands").getConfigurationSection("gms").getBoolean("enabled") == true) {
			...
			if (args.length == 0) {
				if (p.hasPermission("catx.gms")) {
					Player me = p.getServer().getPlayer(p.getName());
					me.sendMessage(handler.getCaption("GamemodeSet").replace("{0}", "survival"));
					me.setGameMode(GameMode.SURVIVAL);
					return true;
				} else {
					p.sendMessage(handler.getCaption("NoPermission"));
				}
			} else {
				if (p.hasPermission("catx.gms.others")) {
					Player other = p.getServer().getPlayer(args[0]);
					other.sendMessage(handler.getCaption("GamemodeSet").replace("{0}", "survival"));
